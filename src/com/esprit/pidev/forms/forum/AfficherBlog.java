 ...
 * @author ASUS
 */
public class AfficherBlog extends BaseForm
{

    public AfficherBlog(Resources res) 
    {
      ...
      //-------------------------------------------------------------------------------
        ...
        ArrayList<Forum> List = new ForumService().getAllEvents();
        Label title=new Label("t");
        Label content=new Label("v");
        for (int i = 0; i<List.size(); i++) 
        {
            ...
            mBtn.setTextLine1(List.get(i).getTitle());
            mBtn.setTextLine2(List.get(i).getContent());
            int id=List.get(i).getId();
            String c=List.get(i).getContent();
            String t=List.get(i).getTitle();
            String im=List.get(i).getImage();
            FontImage.setMaterialIcon(mBtn, FontImage.MATERIAL_ADD_COMMENT);
            SwipeableContainer sousou=  new SwipeableContainer(cntr1, cntr, mBtn);
            listRec.addComponent(sousou);
            mBtn.addActionListener(al->{
                System.out.println(id);
                new BlogForm(res,id,t,c,im).show();
            });
